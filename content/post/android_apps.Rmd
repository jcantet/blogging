...
Toutes les applications ne se prêtent pas à un modèle économique fondé sur les publicités, par exemple les applications de personnalisation. Une fois le nouveau thème de l'appareil installé, l'apparence du clavier renouvelée ou la police d'écriture modifiée, l'utilisateur ne revient pas dans l'application. Il ne va alors pas visionné d'annonces commerciales.
Fixer un prix d'achat est aussi un signal envoyé au potentiel utilisateur, qui peut s'interpréter comme un gage de qualité : l'appplication est travaillée, pensée et désignée par des professionnels, avec un contenu de qualité. Tous ces intervenants doivent être rémunérés, donc le service est vendu à un prix donné. On peut imaginer que c'est une des raisons pour laquelle une application de médecine sur cinq est payante.  
